...
## 2026-09-24 | 我让它写的"检查"，把论文自己的数据判成了坏的

**我当时的 prompt**：核查 ToT 日志里的候选是不是真的在解这道题——写个脚本判断。

**Agent 的原话**（第一版跑完）："官方 gpt-4 ToT 日志（论文自己的数据）也 **FAIL 了 14/15**"——
听起来像是论文的数据也有问题。

**我怎么发现的**：让它把失败原因打出来，发现全是 `left 不符`。那是模型的**标注写得潦草**
（`left:` 里多写了个已经用掉的数），而 `test_output` **根本不看中间标注**。
改掉这条判据后，**第二次仍然 FAIL 14/15**，原因变成"第 4 步用了池外数字"——
而 `self.steps=4`，**没解出 24** 的轨迹第 4 步必然如此（3 步操作后池里只剩 1 个数，做不了第 4 次合并），
那是"正常的解不出"，不是"跑偏"。第三版只看**第 1 步**，官方 gpt-4 日志才 **PASS 75/75**。

**真相**：判据错了两版。一个把论文自己的数据判成坏掉的检查，是**检查错了**，不是数据坏了。
真正定位到 bug（6/15 题示例回显）全靠第三版。

**教训**：任务书 §8.2 说"先写一个能失败的检查"——还得再加一句：**检查本身也要被验证**。
验证方式就是拿一份**已知正确**的数据当对照组（这次用的是官方 gpt-4 日志）。
**"能失败的检查"不等于"判据正确的检查"**；一个从不失败、或者到处都失败的检查，都要先怀疑检查本身。
